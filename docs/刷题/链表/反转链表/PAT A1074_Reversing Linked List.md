**思路**：由于PAT输入数据的特殊性，本题考虑用静态链表来实现，注意输入数据中可能有无效的结点，因此需要县遍历一次并且确定有效结点数order，并且为每个有效结点附上顺序标记。根据顺序标记进行排序后直接对静态链表数组中前order个结点进行输出，每k个结点倒序输出，最后一组若不满足k个结点则按正常顺序输出。
